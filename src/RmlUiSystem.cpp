...
static const char* kGameBody = R"RML(
    <!-- Schwebende Bilder (GameUI::ShowPicture, XP 231) - bis zu 8 Stueck --
    Hinweis: die REIHENFOLGE im Dokument bestimmt die Schicht (spaeter = vorne) -->
    ...
    <img id="pic4" src="" style="position:absolute; display:none; z-index: 1;"/>
    <img id="pic5" src="" style="position:absolute; display:none; z-index: 1;"/>
    <img id="pic6" src="" style="position:absolute; display:none; z-index: 1;"/>
    <img id="pic7" src="" style="position:absolute; display:none; z-index: 1;"/>
...
public:
    ...
    float stY[6] = {};
    bool stOn[6] = {};
    std::string picSrc[8]; // zuletzt gesetzte <img>-Quellen (Pic-Mirror)
    int gold = 0;
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (i < (int)pics.size() && !pics[i].filename.empty()) {
                ...
                if (!expired && !src.empty() && std::filesystem::exists(src)) {
                    ...
                    rotation = pic.rotation; // Grad (XP: Bild drehen, 232)
            ...
            if (visible) {
                ...
                // Drehung (XP 232) um die Bildmitte; translate zentriert weiter
                std::string tf = centered ? "translate(-50%, -50%)"
                                          : "translate(0px, 0px)";
                if (rotation != 0.0f) {
                    tf += " rotate(" + std::to_string((int)rotation) + "deg)";
                    el->SetProperty("transform-origin", "center center");
                }
